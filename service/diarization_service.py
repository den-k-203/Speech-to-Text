...
def init_diarization_pipeline():
    ...
    pipeline = Pipeline.from_pretrained(LOCAL_PATH_TO_MODEL)
...
def process_audio_segments(path_to_audio, output_json_path="output.json"):
    ...
    for turn, _, speaker_label in diarization.itertracks(yield_label=True):
        ...
        elif start >= end or end > len(full_audio_np):
            logger.warning("Пропущено сегмент: некоректний діапазон.")
        else:
            audio_segment = full_audio_np[start:end]

            if audio_segment.size == 0:
                logger.warning("Сегмент порожній.")
            elif audio_segment.ndim != 1:
